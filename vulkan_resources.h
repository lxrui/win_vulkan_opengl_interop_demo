...
namespace vulkanResources {
	...
	inline const std::string versionToStringNv(const uint32_t version) {
		std::stringstream ss;
		uint32_t major = (version >> 22) & 0x3ff;
		uint32_t minor = (version >> 14) & 0x0ff;
		uint32_t secondaryBranch = (version >> 6) & 0x0ff;
		uint32_t tertiaryBranch = (version) & 0x003f;		
		ss << major << "." << minor << "." << secondaryBranch << "." << tertiaryBranch;
		return ss.str();
	}
...
#define VK_CALL(vk_fun) 																			\
do { 																								\
	VkResult vk_ret = vk_fun; 																		\
	if (VK_SUCCESS != vk_ret) { 																	\
		std::cout << #vk_fun << "failed: " << vulkanResources::resultString(vk_ret) << std::endl; 	\
		return vk_ret; 																				\
	} 																								\
} while(false)